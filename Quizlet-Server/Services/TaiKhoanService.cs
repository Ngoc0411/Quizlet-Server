...
namespace Quizlet_Server.Services
{
    public class TaiKhoanService : ITaiKhoanService
    {
        ...
        public IQueryable<TaiKhoan> GetDanhSachTaiKhoan(string keywords)
        {
            ...
            if (!string.IsNullOrEmpty(keywords))
            {
                ...
                query = query.Where(tk => tk.TenTaiKhoan.ToLower().Contains(keywords)
                                    || tk.Email.ToLower().Contains(keywords)
                                    );
            }
            return query;
        }
        ...
        public TaiKhoan UpdateTaiKhoan(int taiKhoanId, TaiKhoan taiKhoan)
        {
            if (appDbContext.TaiKhoans.Any(sp => sp.TaiKhoanID == taiKhoanId))
            {
                ...
                currentTaiKhoan.Password = taiKhoan.Password;
                currentTaiKhoan.TenTaiKhoan = taiKhoan.TenTaiKhoan;
